 ...
 * SIDE PANEL LOGIC
 *
 * Handles the UI for DeepWatch: video detection, transcript analysis,
 * rendering results, chat, and export features.
 */
...
async function checkCurrentTab() {
  try {
    ...
    debugLog("[DeepWatch Panel] Found tab:", tab?.id, tab?.url);
    ...
    if (videoId) {
      ...
      try {
        ...
        debugLog("[DeepWatch Panel] getVideoInfo result:", result);
        ...
        console.error("[DeepWatch Panel] getVideoInfo error:", e);
        currentVideoTitle = "";
        currentChannelName = "";
  ...
  try {
    for (
      ...
      try {
        ...
        console.error("[DeepWatch] Interface batch error:", error);
    ...
    console.error("[DeepWatch] Interface translation error:", error);
...
function renderAnalysisResults(analysis) {
  ...
  (analysis.chapters || []).forEach((chapter, index) => {
    ...
    li.addEventListener("click", () => {
      debugLog(
        "[DeepWatch Panel] Chapter clicked:",
        chapter.timestamp,
...
async function saveQuoteAsNote(quote, btn) {
  ...
  try {
    ...
    if (result.success) {
      ...
      console.error("[DeepWatch] Save quote as note failed:", result.error);
    ...
    console.error("[DeepWatch] Save quote as note error:", error);
    btn.textContent = "Error";
    setTimeout(() => {
      btn.textContent = originalText;
      btn.disabled = false;
...
function exportTranscript() {
  ...
  exportText += `TRANSCRIPT:\n\n${transcriptContent}\n`;
  exportText += `\n${"—".repeat(60)}\n`;
  exportText += `Exported by DeepWatch\n`;
...
async function triggerAnalysis() {
  ...
  try {
    ...
    console.error("[DeepWatch Panel] Analysis error:", error);
    if (chapterList)
...
async function seekTo(seconds) {
  debugLog("[DeepWatch Panel] seekTo called with:", seconds);
  if (seconds === undefined || seconds === null) {
    debugLog("[DeepWatch Panel] seekTo aborted - no seconds value");
    return;
  }
  ...
  try {
    ...
    if (youtubeTabId) {
      try {
        ...
        debugLog("[DeepWatch Panel] seekTo direct success");
        return;
      } catch (directErr) {
        debugLog(
          "[DeepWatch Panel] Direct seekTo failed, falling back to relay:",
          directErr.message,
        );
      }
    }
    ...
    debugLog("[DeepWatch Panel] seekTo relay result:", result);
  } catch (error) {
    console.error("[DeepWatch Panel] seekTo error:", error);
  }
}
  ...
  tooltip
    ...
    .addEventListener("click", async (event) => {
      ...
      try {
        ...
        console.error("[DeepWatch] Save selected note error:", error);
...
async function evictOldCacheEntries(maxEntries) {
  try {
    ...
    if (toRemove.length > 0) {
      ...
      debugLog(`[DeepWatch] Evicted ${toRemove.length} old cache entries`);
    }
  } catch (error) {
...
async function loadNotes(videoId) {
  try {
    ...
    console.error("[DeepWatch Panel] Load notes error:", error);
  }
}
...
async function deleteNote(noteId) {
  try {
    ...
    console.error("[DeepWatch Panel] Delete note error:", error);
  }
}
...
async function loadTranscriptViewState(videoId) {
  ...
  try {
    ...
    console.error("[DeepWatch] Reading position load error:", error);
    return null;
  }
}
...
async function saveTranscriptViewState(videoId, scrollTop) {
  ...
  try {
    ...
    console.error("[DeepWatch] Reading position save error:", error);
  }
}
...
/**
 * Clears the Chat tab's UI and in-memory session when a new video loads.
 * The previous video's session was already persisted to chat_cache_<id> on
 * every message sent, so nothing is lost — this just stops it bleeding
 * into the new video.
 */
...
/**
 * Starts a new chat session for the current video, saving the previous one
 * first. Each video keeps its own list of sessions in chat_cache_<videoId>,
 * matching the Lingo project this feature came from.
 */
async function initNewChatSession(topicHint = "") {
  await saveCurrentChatSessionToStorage();
...
async function saveCurrentChatSessionToStorage() {
  const session = currentChatSession;
  if (!session || !session.messages.length) return;
  const videoId = session.meta?.video_id;
  if (!videoId) return;

  try {
    const key = `chat_cache_${videoId}`;
    const stored = await chrome.storage.local.get(key);
    const sessions = stored[key] || [];
    const index = sessions.findIndex(
      (existing) => existing.meta.session_id === session.meta.session_id,
    );
    if (index !== -1) sessions[index] = session;
    else sessions.push(session);
    await chrome.storage.local.set({ [key]: sessions });
  } catch (error) {
    console.error("[DeepWatch] Failed to save chat session:", error);
  }
}

function addChatMessageToUi(content, role) {
  const messages = document.getElementById("chatMessages");
  if (!messages) return;
...
async function sendChatMessage() {
  ...
  try {
    const result = await chrome.runtime.sendMessage({
      action: "chatMessage",
      messages: currentChatSession.messages.map((message) => ({
        role: message.role,
    ...
    addChatMessageToUi(result.text, "model");
    await saveCurrentChatSessionToStorage();
  } catch (error) {
    addChatMessageToUi(`Error: ${error.message}`, "model");
  } finally {
    isChatSending = false;
...
/**
 * Exports every video's chat sessions as one JSON file, then clears them
 * from storage — a one-time backup, matching the Lingo project's original
 * export behavior.
 */
async function exportChatSessions() {
  ...
  const chatKeys = Object.keys(all).filter((key) =>
  ...
  for (const key of chatKeys) {
    combinedSessions = combinedSessions.concat(all[key]);
  }

  const exportData = {
    export_time: new Date().toISOString(),
    total_videos: chatKeys.length,
  ...
  link.href = url;
  link.download = `DeepWatch_Chat_Export_${Date.now()}.json`;
