...
test("a huge raw caption entry is split into seekable bounded segments", () => {
  const { groupTranscriptEntries } = loadSidepanelHelpers();
...
test("all AI product requests hit Gemini's generateContent shape", async () => {
  ...
  const gemini = loadBackgroundHelpers({
    fetchImpl: successfulFetch(geminiRequests),
  });
  const geminiResult = await gemini.requestAiCompletion({
    maxTokens: 128,
  ...
  assert.equal(geminiResult.text, "translated");
  assert.match(geminiRequests[0].url, /^https:\/\/generativelanguage\.googleapis\.com\/v1beta\/models\/.+:generateContent\?key=test-key$/);
  assert.deepEqual(geminiRequests[0].body.systemInstruction, {
    parts: [{ text: "Be terse." }],
  });
  assert.deepEqual(geminiRequests[0].body.contents, [
    { role: "user", parts: [{ text: "Hello." }] },
  ]);
  assert.equal(geminiRequests[0].body.generationConfig.maxOutputTokens, 128);
  assert.equal(
    geminiRequests[0].body.generationConfig.responseMimeType,
    "application/json",
  );
...
test("Gemini retries one empty transcript JSON response without responseMimeType", async () => {
  const requests = [];
  const helpers = loadBackgroundHelpers({
  ...
  assert.equal(
    requests[0].generationConfig.responseMimeType,
    "application/json",
  );
  assert.equal(
    Object.hasOwn(requests[1].generationConfig, "responseMimeType"),
    false,
  );
  assert.equal(requests[0].generationConfig.maxOutputTokens, 1536);
});
          ...
          candidates: [
            {
              content: {
                parts: [
                  {
                    text: '{"segments":[{"id":"note-1","text":"\u4e2d\u6587\u7b14\u8bb0\u3002"}]}',
                  },
                ],
              },
            },
