...
test("Settings copy covers English and Simplified Chinese", () => {
  assert.equal(options.translate("en", "pageTitle"), "DeepWatch Settings");
  assert.equal(options.translate("zh-CN", "pageTitle"), "DeepWatch 设置");
...
test("customization guidance is concise and has a visible placeholder reminder", () => {
  ...
  assert.equal((steps[1].match(/<li\b/g) || []).length, 2);
...
test("customization prompt switches languages and preserves technical values", () => {
  ...
  assert.match(
    englishPrompt,
    /Keep Gemini-only request fields and retry behavior isolated to Gemini\. Handle provider-specific rules separately so one provider does not affect another\./,
  );
  assert.match(
    chinesePrompt,
    /Gemini 专用的请求参数和重试逻辑继续只用于 Gemini。新服务的专属规则请单独处理，避免相互影响。/,
  );
