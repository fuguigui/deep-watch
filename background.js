 ...
 * 2. Fetching video transcripts directly from the site (no third-party API)
 * 3. Calling Gemini to analyze the transcript
...
importScripts("settings.js", "transcript/youtube.js");
...
chrome.storage.local
  ...
  .catch((error) =>
    console.warn("[DeepWatch] Could not restrict storage access:", error),
  );
...
/**
 * Converts our internal { messages: [{role, content}] } shape (an
 * OpenAI-style shape, kept so every call site below stays unchanged) into
 * Gemini's generateContent request body: a systemInstruction plus a
 * contents array using "user"/"model" roles instead of "user"/"assistant".
 */
function buildGeminiRequestBody({ messages, maxTokens, temperature, responseFormat }) {
  const systemParts = [];
  const contents = [];
  for (const message of messages || []) {
    ...
    // Accept both "assistant" (the OpenAI-style role callers historically
    // used) and "model" (Gemini's own name, which the Chat tab sends
    // directly) as a prior AI turn — everything else is a user turn.
    const role =
 ...
 * you to every tab. To make DeepWatch behave like a YouTube-only tool, we
...
async function closePanelForTab(tabId, windowId) {
  ...
  try {
    // This closes the tab-specific panel used by DeepWatch.
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "fetchTranscript") {
    // The transcript adapter reads captions straight from the video's own
    // tab (see transcript/youtube.js), so it needs that tab's ID — either
    // supplied by the side panel, or the sending tab's own ID.
    const tabId = message.tabId || sender.tab?.id;
    handleFetchTranscript(message.videoId, tabId)
      .then(sendResponse)
  ...
  if (message.action === "explainSelection") {
    // Explain selected text using Gemini.
  ...
  if (message.action === "chatMessage") {
    // Chat tab: send the conversation so far and get Gemini's reply.
    handleChatMessage(message.messages, message.videoTitle)
      .then(sendResponse)
  ...
  // Translation: send content to Gemini.
  ...
  if (message.action === "openSidePanel") {
    ...
    debugLog("[DeepWatch BG] openSidePanel requested from tab:", tabId);
    ...
    if (tabId) {
      ...
      chrome.sidePanel
        ...
        .catch((err) => {
          console.error("[DeepWatch BG] openSidePanel error:", err);
        });
    } else {
        ...
        .then((tabs) => {
          if (tabs[0]) {
            ...
            chrome.sidePanel.open({ tabId: tabs[0].id }).catch((err) => {
              console.error(
                "[DeepWatch BG] openSidePanel fallback error:",
                err,
              );
            });
          }
  ...
  if (message.action === "relayToContent") {
    debugLog("[DeepWatch BG] Relay request:", message.payload?.action);
    (async () => {
      try {
        ...
        debugLog(
          "[DeepWatch BG] Active tab in last focused window:",
          tabs.length,
          tabs[0]?.url,
        );
        ...
        if (!tabs[0] || !tabs[0].url?.includes("youtube.com")) {
          ...
          debugLog("[DeepWatch BG] Active YouTube tabs:", tabs.length);
        }

        // Still nothing? Try any YouTube tab
        if (!tabs[0]) {
          tabs = await chrome.tabs.query({ url: "https://www.youtube.com/*" });
          debugLog("[DeepWatch BG] Any YouTube tabs:", tabs.length);
        }

        if (tabs[0]) {
          debugLog(
            "[DeepWatch BG] Sending to tab:",
            tabs[0].id,
            "URL:",
            tabs[0].url,
          );
          ...
          debugLog("[DeepWatch BG] Got response from content:", response);
          sendResponse({ success: true, response });
        } else {
          debugLog("[DeepWatch BG] No YouTube tab found");
          sendResponse({ success: false, error: "No YouTube tab found" });
        }
      } catch (err) {
        console.error("[DeepWatch BG] Relay error:", err.message);
...
async function getPlayerVideoDetails(tabId) {
  try {
    ...
    console.warn("[DeepWatch BG] Player details unavailable:", e.message);
    return null;
  }
}

// ============================================================
// TRANSCRIPT FETCHING — free, direct from the site, no third-party API
// ============================================================

/**
 * Fetches the transcript for the video open in `tabId`.
 *
 * This delegates to a per-site adapter (see transcript/youtube.js for how
 * YouTube's is implemented and why). Today only YouTube is supported;
 * adding another site later means adding another adapter file and
 * dispatching to it here by tab URL, not changing any caller of this
 * function — every caller already only deals with the shared
...
async function handleAnalyzeTranscript(
  ...
  try {
    ...
    debugLog("[DeepWatch] Requesting video analysis", settings.aiModel);
 ...
 *
 * @param {Object} analysis - The parsed analysis from Gemini
...
/**
 * Explains selected text using Gemini.
...
async function handleSaveNote(
  ...
  try {
    ...
    try {
      ...
      debugLog("[DeepWatch] No cached transcript, fetching...");
    }

    // If no cached transcript, fetch it
    if (!transcript) {
      const transcriptResult = await handleFetchTranscript(videoId, tabId);
    ...
    console.error("[DeepWatch] Save note error:", error);
    return { success: false, error: error.message };
  }
}

/**
 * Cleans up transcript lines using Gemini.
...
async function cleanupNoteText(
  ...
  try {
    debugLog("[DeepWatch] Requesting note cleanup");
    const variables = {
      videoTitle: videoTitle || "Unknown",
      fullContext,
    ...
    try {
      ...
      console.warn(
        "[DeepWatch] JSON parse failed for note, stripping preambles:",
        parseError,
      );
    ...
    console.error("[DeepWatch] Cleanup error:", e);
  }
...
async function handleExplainSelection(
  ...
  try {
    ...
    debugLog("[DeepWatch] Requesting selection explanation");
...
// ============================================================
// CHAT — free-form conversation from the side panel's Chat tab
// ============================================================

/**
 * Sends the Chat tab's conversation so far to Gemini and returns its reply.
 *
 * Unlike the other AI features here, the caller (sidepanel.js) already
 * tracks the whole conversation for the current session — every prior
 * user/model turn is passed in `messages` — so this just prepends the
 * system instruction and forwards it.
 */
async function handleChatMessage(messages, videoTitle) {
  try {
    const settings = await getSettings();
    if (!settings.aiApiKey) {
      return {
    ...
    const systemPrompt = await loadPromptSection("chat.md", "System prompt", {
      videoTitle: videoTitle || "Unknown",
    });
    ...
    debugLog("[DeepWatch] Requesting chat reply");
    ...
    return {
      success: false,
      error: error.message || "Failed to get a chat reply",
      code: error.code,
    };
  }
}
...
/**
 * Translates content using Gemini.
...
async function handleTranslateContent(
  ...
  try {
    ...
    // Gemini JSON mode can rarely return an empty content string. The prompt
    ...
    console.error("[DeepWatch] Translation error:", error);
    return { success: false, error: error.message || "Translation failed" };
  }
}

/**
 * Makes a single Gemini call for translation.
